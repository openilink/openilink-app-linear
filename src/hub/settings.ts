/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Linear API Key 配置
 */
...
export async function handleSettingsSave(
  ...
  if (linearApiKey) newConfig.linear_api_key = linearApiKey;
...
function renderConfigForm(
  ...
  const maskedApiKey = maskValue(cfg.linear_api_key || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Linear — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Linear 设置</h1>
    <p class="desc">修改您的 Linear API Key 配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 API Key</dt>
      <dd>${maskedApiKey || "（未配置）"}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="linear_api_key" type="password" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://linear.app/settings/api" target="_blank">Linear Settings → API</a> 创建</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-linear">自行部署</a>本 App</li>
        </ul>
      </div>
